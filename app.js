...
(function () {
  ...
  function genNick() {
    ...
    return a[Math.floor(Math.random() * a.length)] + '_' + Math.floor(1000 + Math.random() * 9000);
  }
  function nickInitials(n) {
    var p = (n || '').trim().split(/[\s_-]+/).filter(Boolean);
    return ((p[0] || 'G').charAt(0) + (p[1] || 'W').charAt(0)).toUpperCase();
  }
  function syncAvatar() {
    var n = els.nickInput.value.trim();
    els.myAvatar.textContent = n ? nickInitials(n) : 'GW';
  }
  function loadPrefs() {
    /* persistent guest account — identity survives visits (airtalk-style) */
    var savedNick = localStorage.getItem('gw_nick');
    if (!savedNick) { savedNick = genNick(); localStorage.setItem('gw_nick', savedNick); }
    ...
    syncAvatar();
    if (localStorage.getItem('gw_seen') === '1') {
      els.setupSub.textContent = 'Welcome back, ' + els.nickInput.value.trim() + ' 👋 Your guest account is saved on this device.';
    } else {
      localStorage.setItem('gw_seen', '1');
    }
  }
  ...
  function startSearch() {
    ...
    if (!els.nickInput.value.trim()) { els.nickInput.value = genNick(); syncAvatar(); }
  ...
  function cacheEls() {
    var ids = ['banner', 'bannerText', 'bannerX', 'connState', 'tbOnline', 'tbCap', 'soundBtn', 'prefsBtn',
      ...
      'myAvatar', 'setupSub', 'guestNote', 'switchGuest',
  ...
  function bindUI() {
    ...
    els.nickDice.addEventListener('click', function () {
      els.nickInput.value = genNick();
      localStorage.setItem('gw_nick', els.nickInput.value);
      syncAvatar();
    });
    els.nickInput.addEventListener('input', syncAvatar);
    els.switchGuest.addEventListener('click', function () {
      var n = genNick();
      els.nickInput.value = n;
      localStorage.setItem('gw_nick', n);
      syncAvatar();
      toast('New guest account: ' + n, 'ok');
    });
