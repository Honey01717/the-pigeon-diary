...
if (HAS_PUBLIC) {
  app.use(express.static(PUBLIC_DIR, {
    ...
    setHeaders: (res, filePath) => {
      if (/\.html?$/.test(filePath) || filePath.endsWith('sw.js') || filePath.endsWith('manifest.json')) {
        res.setHeader('Cache-Control', 'no-cache'); // pages/SW/manifest must be fresh after every deploy
      }
    }
  }));
} else {
  ...
  app.use((req, res, next) => {
    ...
    if (!file) return next();
    if (/\.html?$/.test(file) || file.endsWith('sw.js') || file.endsWith('manifest.json')) {
