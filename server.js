...
const matchmakingQueue = []; // socketIds looking for a voice call
const textMatchmakingQueue = []; // socketIds looking for a 1-on-1 text chat
...
function matchAvailableTextUsers() {
  if (siteConfig.maintenanceMode || textMatchmakingQueue.length < 2) return;

  while (textMatchmakingQueue.length >= 2) {
    const user1Id = textMatchmakingQueue.shift();
    const user1 = activeUsers.get(user1Id);
    if (!user1 || user1.status !== 'in_text_queue') continue;

    const user2Id = textMatchmakingQueue.shift();
    const user2 = activeUsers.get(user2Id);
    if (!user2 || user2.status !== 'in_text_queue') {
      textMatchmakingQueue.unshift(user1Id);
      continue;
    }

    const chatId = `text_${Date.now()}_${Math.random().toString(36).substr(2, 6)}`;
    user1.status = 'in_text_chat';
    user1.currentCallId = chatId;
    user1.callPartnerId = user2Id;

    user2.status = 'in_text_chat';
    user2.currentCallId = chatId;
    user2.callPartnerId = user1Id;

    io.to(user1Id).emit('text-chat-matched', {
      chatId,
      partner: {
        id: user2Id,
        nickname: user2.nickname,
    ...
    io.to(user2Id).emit('text-chat-matched', {
      chatId,
      partner: {
        id: user1Id,
        nickname: user1.nickname,
...
io.on('connection', (socket) => {
  ...
  socket.on('start-call-search', () => {
    ...
    const user = activeUsers.get(socket.id);
    if (!user) {
      socket.emit('error-msg', { message: 'Please set your profile first.' });
      return;
    }
  ...
  // 3.1 Start 1-on-1 Random Text Chat Search
  socket.on('start-text-search', () => {
    const user = activeUsers.get(socket.id);
    if (!user) return;

    if (user.status === 'in_text_chat' || user.status === 'in_call') {
      return;
    }

    user.status = 'in_text_queue';
    if (!textMatchmakingQueue.includes(socket.id)) {
      textMatchmakingQueue.push(socket.id);
    }

    socket.emit('text-search-status', { status: 'searching', queuePosition: textMatchmakingQueue.length });
    broadcastLobbyState();
    matchAvailableTextUsers();
  });

  // 3.2 Cancel Text Search
  socket.on('cancel-text-search', () => {
    const user = activeUsers.get(socket.id);
    if (user && user.status === 'in_text_queue') {
      user.status = 'idle';
      const idx = textMatchmakingQueue.indexOf(socket.id);
      if (idx !== -1) textMatchmakingQueue.splice(idx, 1);
      socket.emit('text-search-status', { status: 'idle' });
      broadcastLobbyState();
    }
  });

  // 3.3 Next Text Stranger
  socket.on('next-text-stranger', () => {
    const user = activeUsers.get(socket.id);
    if (!user) return;
    ...
    if (user.callPartnerId) {
      ...
      if (partner) {
        ...
        io.to(user.callPartnerId).emit('text-chat-ended', { reason: 'partner_skipped' });
      }
    }
  ...
  // 3.4 Disconnect / Leave Text Chat
  socket.on('leave-text-chat', () => {
    ...
    if (user.callPartnerId) {
      ...
      if (partner) {
        ...
        partner.callPartnerId = null;
        partner.currentCallId = null;
        io.to(user.callPartnerId).emit('text-chat-ended', { reason: 'partner_left' });
      }
    }
    ...
    user.callPartnerId = null;
    user.currentCallId = null;
    socket.emit('text-chat-ended', { reason: 'you_left' });
    broadcastLobbyState();
  });
  ...
  // 5. WebRTC Signaling with Partner Verification Shield
  socket.on('webrtc-offer', (payload) => {
    const user = activeUsers.get(socket.id);
    if (!user || user.status !== 'in_call' || user.callPartnerId !== payload.targetId) return;
  ...
  socket.on('audio-speaking', (payload) => {
    const user = activeUsers.get(socket.id);
    if (user && user.status === 'in_call' && user.callPartnerId) {
  ...
  // 8.1 DEDICATED IN-CALL LIVE WHISPERS (STRICT 1-ON-1 ISOLATION SHIELD)
  socket.on('send-incall-whisper', (data) => {
    if (!checkRateLimit(socket.id, 10, 2000)) {
      socket.emit('error-msg', { message: 'Typing too fast! Please slow down.' });
      return;
    }

    const user = activeUsers.get(socket.id);
    // Strict Verification: user MUST be in a call with a valid partner
    if (!user || user.status !== 'in_call' || !user.callPartnerId || !data) {
      socket.emit('error-msg', { message: 'Private call session required to send in-call whispers.' });
      return;
    }

    const partner = activeUsers.get(user.callPartnerId);
    if (!partner || partner.status !== 'in_call' || partner.callPartnerId !== socket.id) {
      socket.emit('error-msg', { message: 'Partner is no longer connected to this private call.' });
      return;
    }

    const whisperType = data.type || 'text';
    let safeText = '';
    if (data.text) safeText = sanitizeHtml(String(data.text)).substring(0, 500);
    ...
    const whisperPayload = {
      ...
      caption: data.caption ? sanitizeHtml(String(data.caption)).substring(0, 300) : '',
      audioData: data.audioData || null,
      duration: data.duration || 0,
      timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })
    };

    // STRICT 1-ON-1 ROUTING ONLY TO THE VERIFIED CALL PARTNER
    io.to(user.callPartnerId).emit('receive-incall-whisper', whisperPayload);
    socket.emit('incall-whisper-ack', whisperPayload);
  });

  // 8. Real-time Public/Friend Chat with Anti-XSS & Anti-Spam
